...
+ (dispatch_semaphore_t)semaphore {
    static dispatch_semaphore_t semaphore_ = nil;
...
+ (void)_throttle:(NSTimeInterval)threshold type:(GCDThrottleType)type queue:(dispatch_queue_t)queue key:(NSString *)key block:(GCDThrottleBlock)block {
    ...
    if (!block) return;
    dispatch_source_t source = [self sourceForKey:key];
    ...
    [self setSource:source withKey:key];
}

+ (void)setSource:(dispatch_source_t)source withKey:(NSString *)key {
    dispatch_semaphore_wait([self semaphore], DISPATCH_TIME_FOREVER);
    [[self scheduledSources] setObject:source forKey:key];
    dispatch_semaphore_signal([self semaphore]);
}

+ (dispatch_source_t)sourceForKey:(NSString *)key {
    dispatch_source_t source = nil;
    dispatch_semaphore_wait([self semaphore], DISPATCH_TIME_FOREVER);
    source = [self scheduledSources][key];
    dispatch_semaphore_signal([self semaphore]);
    return source;
}

+ (void)removeSourceWithKey:(NSString *)key {
    dispatch_semaphore_wait([self semaphore], DISPATCH_TIME_FOREVER);
    [[self scheduledSources] removeObjectForKey:key];
    dispatch_semaphore_signal([self semaphore]);
}
